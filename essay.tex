...
Since $Q$ and $P$ have the same truth value, the expression with `$Q$' must have the same referent as it's counterpart in \ref{setP} (`the set of all objects which are identical with themselves and (for which) $P$ obtains').
...
\footnotetext{Frege was not a correspondence theorist, taking truth as undefinable and primitive (because definitions presuppose themselves to be true, and so any definition of truth is circular).
\parencite[128-129]{LogicFrege} 
So this cannot be taken as an example of a slingshot-compatible correspondence theory.
However, that Frege did not see this conclusion as problematic, and differentiated sentences by their sense is instructive, as I think a slingshot-compatible correspondence theory would proceed on similar lines.
Frege shows us that such a theory as slingshot-compatible correspondence theory \emph{would have to be} is possible.
}
